...
function durationParts(totalMinutes: number | null) {
...
export default function PlanPage() {
  ...
  const [activityTravelMode, setActivityTravelMode] = useState<TravelMode | null>(null);
  const [activityTags, setActivityTags] = useState("");
  const [activityNotes, setActivityNotes] = useState("");
  const [editingActivity, setEditingActivity] = useState<ActivitySummary | null>(null);
  const [editTravelMode, setEditTravelMode] = useState<TravelMode | null>(null);
  ...
  return <main className="app-shell">
    ...
    <section className="card section-card">
      ...
      {showActivityForm && <form className="form-grid subcard" onSubmit={(event: FormEvent) => {
        ...
        void mutate(() => createActivity(session.appJwt!, planId, { name: activityName.trim(), description: activityDescription || undefined, address: activityAddress || undefined, estimated_cost_cents: cents, estimated_duration_minutes: duration, travel_mode: activityTravelMode ?? undefined, tags: activityTags.split(",").map((tag) => tag.trim()).filter(Boolean), notes: activityNotes || undefined, client_operation_id: crypto.randomUUID() })); setShowActivityForm(false);
        setActivityName(""); setActivityDescription(""); setActivityAddress(""); setActivityCost(""); setActivityHours(""); setActivityMinutes(""); setActivityTravelMode(null); setActivityTags(""); setActivityNotes("");
      }}>
        ...
        <fieldset className="travel-selector form-span"><legend>Travel mode <span className="optional">Optional</span></legend>{travelModes.map((mode) => <button type="button" className={`travel-mode ${activityTravelMode === mode.value ? "selected" : ""}`} key={mode.value} aria-label={`Travel by ${mode.label}`} aria-pressed={activityTravelMode === mode.value} disabled={disabled} onClick={() => setActivityTravelMode(activityTravelMode === mode.value ? null : mode.value)}><span aria-hidden="true">{mode.emoji}</span>{mode.label}</button>)}</fieldset>
