...
export default function PlanPage() {
  ...
  return <main className="app-shell">
    ...
    <section className="summary-grid" aria-label="Plan overview">
      <div className="card stat"><span>Date range</span><strong>{plan.starts_on ? `${readableDate(plan.starts_on)} – ${readableDate(plan.ends_on)}` : "Not set"}</strong></div>
      <div className="card stat"><span>Budget</span><strong>{plan.budget_cents === null ? "Not set" : formatCents(plan.budget_cents)}</strong></div>
      <div className="card stat"><span>Members</span><strong>{snapshot.members.length}</strong></div>
      <div className="card stat"><span>Activities</span><strong>{plan.activities.length}</strong></div>
      <div className="card stat"><span>Itinerary</span><strong>{itinerary.length} items</strong></div>
      <div className="card stat"><span>Status</span><strong>{plan.status}</strong></div>
    </section>
    ...
    <section className="card section-card">
      <div className="section-heading"><div><h2>Activities</h2><p className="muted small">Collect ideas, vote, and discuss the details.</p></div><button className="btn" disabled={disabled} type="button" onClick={() => setShowActivityForm((value) => !value)}>{showActivityForm ? "Cancel" : "+ Add activity"}</button></div>
      {showActivityForm && <form className="form-grid subcard" onSubmit={(event: FormEvent) => {
        ...
        <label className="field">Duration minutes <span className="optional">Optional</span><input value={activityDuration} inputMode="numeric" onChange={(event) => setActivityDuration(event.target.value)} disabled={disabled} /></label>
      ...
      <div className="stack" style={{ marginTop: showActivityForm ? 16 : 0 }}>{plan.activities.map((activity) => <article className="activity-card" key={activity.id}><div className="split"><div><h3>{activity.name}</h3><p className="muted small">{activity.description || "No description yet."}</p></div><div className="cluster"><button className="btn btn-secondary" disabled={disabled} onClick={() => setEditingActivity(activity)}>Edit</button>{canManage && <button className="btn btn-danger" disabled={disabled} onClick={() => { if (window.confirm(`Delete ${activity.name}?`)) void mutate(() => deleteActivity(session.appJwt!, planId, activity.id, activity.version)); }}>Delete</button>}</div></div><div className="metadata">{activity.address && <span className="chip">{activity.address}</span>}{activity.estimated_cost_cents !== null && <span className="chip">{formatCents(activity.estimated_cost_cents)}</span>}{activity.estimated_duration_minutes !== null && <span className="chip">{activity.estimated_duration_minutes} min</span>}{activity.tags.map((tag) => <span className="chip" key={tag}>#{tag}</span>)}</div>{activity.notes && <p className="small">{activity.notes}</p>}<div className="split"><div><strong className="small">Yes {activity.yes_votes} · Maybe {activity.maybe_votes} · No {activity.no_votes}</strong>{plan.vote_visibility === "public" && <p className="muted small">Votes: {snapshot.votes.filter((vote) => (vote as { activity_id?: string }).activity_id === activity.id).map((vote) => { const record = vote as { user_id?: string; vote?: string }; return `${displayMember(snapshot, String(record.user_id))} (${String(record.vote)})`; }).join(", ") || "No votes yet"}</p>}{plan.vote_visibility === "anonymous" && <p className="muted small">Votes are anonymous; only totals and your own choice are shown.</p>}</div><div className="vote-control" aria-label={`Vote on ${activity.name}`}>{(["yes", "maybe", "no"] as const).map((vote) => <button className={`btn ${activity.vote === vote ? "selected" : ""}`} key={vote} disabled={disabled} onClick={() => void mutate(() => voteActivity(session.appJwt!, planId, activity.id, vote))}>{activity.vote === vote ? `${vote} selected` : vote}</button>)}</div></div>
