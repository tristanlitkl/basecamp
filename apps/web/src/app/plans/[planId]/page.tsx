...
function DisclosureSection({ id, title, summary, children, defaultOpen = true, className = "", actions }: DisclosureSectionProps) {
  ...
  return <section className={`section-card disclosure-section ${className}`} id={id}>
...
export default function PlanPage() {
  ...
  const [showItineraryComposer, setShowItineraryComposer] = useState(false);
  ...
  return <main className={`app-shell ${styles.planPage}`}><AdventureBackground />
    ...
    <header className="plan-header trip-hero">
      ...
      <div className="plan-title-row">
        <div><p className="eyebrow">Your next escape</p><h1>{plan.title}</h1><p className="trip-context">{plan.travel_notes?.trim() || plan.description?.trim() || "A shared trip, shaped together."}</p><div className="trip-identity"><span>{plan.starts_on ? `${readableDate(plan.starts_on)}${plan.ends_on ? ` — ${readableDate(plan.ends_on)}` : ""}` : "Dates to be decided"}</span><span>{snapshot.members.length} {snapshot.members.length === 1 ? "traveler" : "travelers"}</span><PresenceAvatarStack users={presence} label={`${presence.length} ${presence.length === 1 ? "person" : "people"} here`} /></div></div>
      ...
      <div className="trip-hero-footer vote-visibility-control"><span className={`badge badge-${plan.status}`}>{plan.status}</span><span className={`badge badge-${socket.connectionState}`}><span className="status-dot" />{connectionLabel(socket.connectionState)}</span><span>Vote visibility: <strong>{plan.vote_visibility === "public" ? "Public" : "Anonymous"}</strong></span>{canManage && <button className="btn btn-quiet" disabled={disabled} type="button" onClick={() => void mutate(() => updateVoteVisibility(session.appJwt!, planId, plan.vote_visibility === "public" ? "anonymous" : "public", plan.version))}>Change</button>}</div>
    </header>
    ...
    <div className="page-grid planning-workspace"><div className="planning-primary">
    ...
    <DisclosureSection id="trip-ideas" title="Trip ideas" summary={`${plan.activities.length} ${plan.activities.length === 1 ? "activity" : "activities"} to discuss and vote on.`} actions={<button className="btn" disabled={disabled} type="button" onClick={() => setShowActivityForm((value) => !value)}>{showActivityForm ? "Cancel" : "+ Add activity"}</button>}>
      ...
      {(() => { const itineraryActivityIds = new Set(snapshot.itinerary_items.flatMap((item) => item.activity_id ? [item.activity_id] : [])); const activityCards = plan.activities.map((activity) => {
        ...
        return { activityId: activity.id, card: <article className={`activity-card ${itineraryItem ? "activity-card-in-itinerary" : "activity-card-not-in-itinerary"}`} key={activity.id}>
          ...
          {itineraryItem && <span className="activity-card-accent" aria-hidden="true" />}
    ...
    <section className="card section-card itinerary-day-view" id="itinerary-day-view" aria-labelledby="itinerary-day-view-heading">
      <div className="section-heading"><div><p className="eyebrow">The trip, one stop at a time</p><h2 id="itinerary-day-view-heading">Your itinerary</h2><p className="muted small">A shared, day-by-day running order.</p></div><button className="btn itinerary-composer-trigger" disabled={disabled} onClick={() => setShowItineraryComposer((open) => !open)} type="button">{showItineraryComposer ? "Close composer" : "+ Add itinerary stop"}</button></div>
      {showItineraryComposer && <>
